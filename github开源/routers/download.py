...
async def api_messages(plugin_id: int = 0, article_id: int = 0):
    ...
    async with get_db() as db:
        if article_id:
            cursor = await db.execute(
                "SELECT * FROM messages WHERE status='approved' AND article_id=? ORDER BY created_at ASC",
                (article_id,),
            )
        elif plugin_id:
            cursor = await db.execute(
                "SELECT * FROM messages WHERE status='approved' AND plugin_id=? ORDER BY created_at ASC",
                (plugin_id,),
            )
        else:
            cursor = await db.execute(
                "SELECT * FROM messages WHERE status='approved' ORDER BY created_at ASC LIMIT 50"
            )
...
async def api_create_message(request: Request):
    ...
    async with get_db() as db:
        ...
        await db.execute(
            "INSERT INTO messages (plugin_id, article_id, parent_id, author, content, status) VALUES (?, ?, ?, ?, ?, 'approved')",
