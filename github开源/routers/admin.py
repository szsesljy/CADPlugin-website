...
async def api_approve(request: Request, plugin_id: int):
    ...
    async with get_db() as db:
        ...
        scan_warn = ""
        if old_path.exists():
            scan_result = await scan_file(old_path)
            if not scan_result.get("clean"):
                scan_warn = scan_result.get("virus", "未知")
                logger.warning("审核提醒: 文件可能含病毒 %s | %s", plugin["name"], scan_warn)
    ...
    return {"success": True, "scan_warning": scan_warn}
    ...
    async with get_db() as db:
        for pid in ids:
            ...
            if old_path.exists():
                scan_result = await scan_file(old_path)
                if not scan_result.get("clean"):
                    logger.warning("批量审核提醒: 文件可能含病毒 %s | %s", plugin["name"], scan_result.get("virus", ""))
...
async def api_direct_upload(request: Request):
    ...
    async with get_db() as db:
        cursor = await db.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, 'approved')",
            (name, description, version, author, file.filename, f"approved/{safe_name}", size),
        )
...
async def api_list_messages(request: Request, status: str = "pending"):
    ...
    async with get_db() as db:
        if status == "all":
            cursor = await db.execute(
                "SELECT m.*, p.name AS plugin_name FROM messages m LEFT JOIN plugins p ON p.id=m.plugin_id ORDER BY m.created_at DESC"
            )
        else:
            cursor = await db.execute(
                "SELECT m.*, p.name AS plugin_name FROM messages m LEFT JOIN plugins p ON p.id=m.plugin_id WHERE m.status=? ORDER BY m.created_at DESC",
                (status,),
            )
        return [dict(r) for r in await cursor.fetchall()]
