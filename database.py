    ...
    async with aiosqlite.connect(str(DATABASE_PATH)) as db:
        ...
        await db.execute("""
            CREATE TABLE IF NOT EXISTS plugins (
                ...
                download_mode TEXT DEFAULT 'direct_only',
        ...
        # ── 迁移：为留言表添加 ip_address 列 ──
        try:
            await db.execute("ALTER TABLE messages ADD COLUMN ip_address TEXT DEFAULT ''")
        except Exception:
            pass
        ...
        await db.execute("""
            CREATE TABLE IF NOT EXISTS visitor_ips (
                id               INTEGER PRIMARY KEY AUTOINCREMENT,
                ip_address       TEXT NOT NULL UNIQUE,
                first_visited_at TEXT NOT NULL DEFAULT (datetime('now','localtime')),
                last_visited_at  TEXT NOT NULL DEFAULT (datetime('now','localtime')),
        ...
        # ── 种子数据：板块（仅在表为空时写入） ──
        cursor = await db.execute("SELECT COUNT(*) FROM boards")
        if (await cursor.fetchone())[0] == 0:
            await db.execute(
